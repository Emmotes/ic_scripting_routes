...
function formsUpdateShow() {
	...
	let mType = "M";
	if (stacking === "Melf" && Object.keys(baseForm).includes("MH"))
		mType = "MH";
...
function addSpecInfo(baldric, stacking, tatyana, specsData) {
	...
	if (baldric && stacking === "Melf") {
		txt += `<span style="display:flex;flex-direction:column">`;
		txt += `<h3>Using Melf and Baldric</h3>Requires:`;
		txt +=
			`<ul style="margin-left:-10px">` +
			`<li>Melf to be set to 0/75 in the Level Up addon.</li>` +
			x25 +
			`</ul>`;
		txt += `</span>`;
	}
