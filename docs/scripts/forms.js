...
function decideFormsShowStatus() {
	...
	if (campaign == "" || type == "" || widdle == "")
		...
		if (campaign == "GT")
			disableElements(false,false,disableBaldric,true,true);
		else
			disableElements(false,false,disableBaldric,false,false);
	}
	formsUpdateShow();
}
 
function disableElements(show,tatyana,baldric,featSwap,hybrid) {
	let grey = `color:#444`;
