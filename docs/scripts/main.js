...
function update() {
	...
	comment += spacer;
	comment += `<span class="routesRow"><span class="routesDesc">When there are multiple routes - they are ordered best to worst.</span></span>`;
	comment += spacer;
...
function computeThelloraDist(inputs, Q, E) {
	if (inputs.favour <= 0) return {minSeen: 1, seen: new Set([1])};

	// If definitely favour-capped, no need to simulate.
	if (inputs.resetZone >= inputs.favour * 5) {
		const dist = inputs.favour + 1;
		return {minSeen: dist, seen: new Set([dist])};
	}
...
function renderResults(inputs, brivData, routeData, stackData) {
	...
	// Bad Stack Zone
	const thirdFromEnd = routeData.route.at(-3);
	const maxSafeStackZone = (thirdFromEnd?.zone ?? 1) - 1;
